...
import me.aurora.tool.repository.AliSmsRepo;
import me.aurora.tool.service.AliSmsService;
...
public class AliSmsServiceImpl implements AliSmsService {
    ...
    public ResponseEntity send(AliSmsConfig config, String phone, String code) throws Exception {
        ...
        IClientProfile profile = DefaultProfile.getProfile("cn-hangzhou", config.getAccessKeyId(),
                EncryptHelper.desDecrypt(config.getAccessKeySecret()));
        ...
        try {
            ...
            throw new AuroraException(HttpStatus.HTTP_INTERNAL_ERROR,e.getErrMsg());
        }
        if(sendSmsResponse.getCode() != null && sendSmsResponse.getCode().equals("OK")) {
            return ResponseEntity.ok();
        } else {
            return ResponseEntity.error(AuroraConstant.Sms.get(sendSmsResponse.getCode()));
        }
    }
}
